...
import { AdminAgentNetworkPage } from './pages/admin/AdminAgentNetworkPage';
...
function Dashboard(){
 ...
 return <div className="space-y-5">
   ...
   <section className="grid grid-cols-1 sm:grid-cols-3 gap-3"><div className="jb-card p-4"><div className="text-xs text-slate-400">Available Coins</div><div className="text-2xl font-black mt-1 flex items-center gap-2"><Coins className="w-5 h-5 text-[#df6f8e]"/>{Math.round(Number(user?.walletBalance||0)/1.5).toLocaleString()}</div><div className="text-[10px] text-slate-500 mt-1">1 coin = ₦1.50 (100 coins = ₦150)</div></div><div className="jb-card p-4"><div className="text-xs text-slate-400">Account</div><div className="text-lg font-black mt-1 truncate">@{user?.username}</div><div className="text-[10px] text-slate-500 mt-1">Ready for your next order</div></div><div className="jb-card p-4"><div className="text-xs text-slate-400">My Orders</div><div className="text-2xl font-black mt-1">{orderCount}</div><Link to="/orders" className="text-[10px] text-[#df6f8e] font-black mt-1 inline-flex items-center gap-1">View orders <ArrowRight className="w-3 h-3"/></Link></div></section>
...
function OrdersPage(){
 ...
 <div className="mt-5 space-y-3"><div className="grid grid-cols-2 gap-2"><div className="rounded-xl border border-white/10 bg-white/[.025] p-3"><div className="text-[10px] text-slate-500">Quantity ordered</div><div className="font-black mt-1">{Number(selected.quantity||0).toLocaleString()}</div></div><div className="rounded-xl border border-white/10 bg-white/[.025] p-3"><div className="text-[10px] text-slate-500">Cost</div><div className="font-black mt-1 flex items-center gap-1"><Coins className="w-4 h-4 text-[#df6f8e]"/>{Number(selected.customerCoins||Math.round(Number(selected.amount||0)/1.5)).toLocaleString()}</div></div></div>
 <div className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><div className="text-[10px] text-slate-500">Target</div><div className="text-xs font-bold break-all mt-1">{selected.targetUrl}</div></div> <div className="grid grid-cols-1 gap-2"><div className="rounded-xl border border-white/10 p-3"><span className="text-slate-600">Starting count</span><div className="font-bold text-slate-300 mt-1">{Number(selected.startCount||0).toLocaleString()}</div></div></div>
...
function AdminShell(){const {adminLogout}=useAuth(); return <div className="min-h-screen bg-[#090305] text-white"><header className="border-b border-white/10 bg-[#0d0508]/90 sticky top-0 z-40"><div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between"><Link to="/boris/dashboard" className="font-black">JB BOOST <span className="text-[#df6f8e]">ADMIN</span></Link><button onClick={adminLogout} className="text-xs font-bold text-slate-300">Logout</button></div></header><main className="max-w-7xl mx-auto p-4 sm:p-6 pb-12"><nav className="flex flex-wrap gap-2 mb-5">{[['/boris/dashboard','Overview'],['/boris/agent-network','Agent Network'],['/boris/orders','Orders'],['/boris/providers','Providers'],['/boris/services','Services'],['/boris/users','Users'],['/boris/deposits','Payments'],['/boris/settings','Settings']].map(([p,l])=><Link key={p} to={p} className="px-3 py-2 rounded-xl bg-white/[.04] border border-white/10 text-xs font-bold hover:border-[#a72b50]/60">{l}</Link>)}</nav><Routes><Route path="/dashboard" element={<AdminDashboardPage/>}/><Route path="/agent-network" element={<AdminAgentNetworkPage/>}/><Route path="/orders" element={<AdminOrdersPage/>}/><Route path="/providers" element={<AdminProvidersPage/>}/><Route path="/services" element={<AdminServicesPage/>}/><Route path="/users" element={<AdminUsersPage/>}/><Route path="/deposits" element={<AdminDepositsPage/>}/><Route path="/settings" element={<AdminSettingsPage/>}/><Route path="*" element={<Navigate to="/dashboard" replace/>}/></Routes></main></div>}
