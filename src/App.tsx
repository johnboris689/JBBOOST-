...
function OrdersPage(){
 ...
 <div className="mt-5 space-y-3"><div className="grid grid-cols-2 gap-2"><div className="rounded-xl border border-white/10 bg-white/[.025] p-3"><div className="text-[10px] text-slate-500">Quantity ordered</div><div className="font-black mt-1">{Number(selected.quantity||0).toLocaleString()}</div></div><div className="rounded-xl border border-white/10 bg-white/[.025] p-3"><div className="text-[10px] text-slate-500">Cost</div><div className="font-black mt-1 flex items-center gap-1"><Coins className="w-4 h-4 text-[#df6f8e]"/>{Math.round(Number(selected.amount||0)*2).toLocaleString()}</div></div></div>
 ...
 <div className="grid grid-cols-1 gap-2 text-[10px]"><div className="rounded-xl border border-white/10 p-3"><span className="text-slate-600">Order opened</span><div className="font-bold text-slate-300 mt-1">{fmt(selected.openedAt||selected.createdAt)}</div></div><div className="rounded-xl border border-white/10 p-3"><span className="text-slate-600">Expected completion</span><div className="font-bold text-slate-300 mt-1">{fmt(selected.expectedCompleteAt)}</div></div><div className="rounded-xl border border-white/10 p-3"><span className="text-slate-600">Last progress update</span><div className="font-bold text-slate-300 mt-1">{fmt(selected.lastProgressAt)}</div></div>{selected.completedAt&&<div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3"><span className="text-emerald-400/70">Completed</span><div className="font-bold text-emerald-300 mt-1">{fmt(selected.completedAt)}</div></div>}</div>
 <div className="text-[10px] text-slate-600">Expected completion is an estimate. Actual fulfillment progress is shown only when the order is updated by the fulfillment system.</div>
 </div></div></div>}
 </div>
}
