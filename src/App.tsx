...
function OrdersPage(){
 const [orders,setOrders]=React.useState<any[]>([]); const [loading,setLoading]=React.useState(true); const [tab,setTab]=React.useState<'open'|'closed'>('open');
 React.useEffect(()=>{api.getSocialOrders().then(setOrders).catch(console.error).finally(()=>setLoading(false));},[]);
 const closedStatuses=new Set(['completed','cancelled','failed']);
 const visible=orders.filter(o=>tab==='closed'?closedStatuses.has(String(o.status||'pending').toLowerCase()):!closedStatuses.has(String(o.status||'pending').toLowerCase()));
 return <div className="space-y-4"><div><h1 className="text-2xl font-black">My Orders</h1><p className="text-xs text-slate-400 mt-1">Track your social-media boosting orders.</p></div>
 <div className="grid grid-cols-2 gap-2"><button onClick={()=>setTab('open')} className={`rounded-xl py-3 text-xs font-black border ${tab==='open'?'bg-[#7d1738] border-[#c2476e]/50 text-white':'bg-white/[.025] border-white/10 text-slate-400'}`}>Open Orders ({orders.filter(o=>!closedStatuses.has(String(o.status||'pending').toLowerCase())).length})</button><button onClick={()=>setTab('closed')} className={`rounded-xl py-3 text-xs font-black border ${tab==='closed'?'bg-[#7d1738] border-[#c2476e]/50 text-white':'bg-white/[.025] border-white/10 text-slate-400'}`}>Closed Orders ({orders.filter(o=>closedStatuses.has(String(o.status||'pending').toLowerCase())).length})</button></div>
 <div className="jb-card p-4">{loading?<div className="py-10 text-center text-xs text-slate-500">Loading orders…</div>:visible.length===0?<div className="py-10 text-center text-sm text-slate-500">{tab==='open'?'No open orders.':'No closed orders yet.'}{tab==='open'&&<><br/><Link className="text-[#df6f8e] font-bold inline-block mt-2" to="/services">Browse services</Link></>}</div>:<div className="space-y-2">{visible.map(o=><div key={o.id} className="p-4 rounded-2xl bg-white/[.025] border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div className="text-[10px] uppercase text-[#df6f8e] font-black">{o.platform} • {o.status}</div><div className="font-bold text-sm mt-1">{o.serviceName}</div><div className="text-[10px] text-slate-500 mt-1 break-all">{o.targetUrl} • {Number(o.quantity).toLocaleString()} units</div><div className="text-[9px] text-slate-600 mt-1">{o.createdAt?new Date(o.createdAt).toLocaleString():''}</div></div><div className="text-sm font-black flex items-center gap-1"><Coins className="w-4 h-4 text-[#df6f8e]"/>{Math.round(Number(o.amount)*2).toLocaleString()}</div></div>)}</div>}</div></div>}
