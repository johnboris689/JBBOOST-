...
Support: support@nivocash.app
    ...
    link.href = url;
    link.download = `JB BosterCash_Receipt_${data.reference}.txt`;
    link.click();
    URL.revokeObjectURL(url);
  };

  const handleShareReceipt = async () => {
    const text = `JB Boster Withdrawal Receipt: ₦${data.amount.toLocaleString()} to ${data.recipientName} (${data.bankName}). Ref: ${data.reference}`;
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'JB Boster Receipt',
          text: text,
        });
      } catch (e) {
        handleCopyRef();
      ...
      >
        ...
        <div className="p-6 text-center space-y-5 relative z-10">
          ...
          <div className="bg-gradient-to-br from-[#240A12] to-[#100709] border border-white/10 p-4 rounded-2xl">
            ...
            <div className="flex items-center justify-center gap-1.5 mt-2 text-[11px] text-[#C13A5A] font-bold">
          ...
          <div className="nivo-glass-surface border border-white/10 rounded-2xl p-4 text-left space-y-2.5 text-xs">
            ...
            <div className="flex justify-between items-center py-1 border-b border-white/10">
              ...
              <span className="font-bold text-[#C13A5A] flex items-center gap-1">
            ...
            <div className="flex justify-between items-center py-1 border-b border-white/10">
              ...
              <div className="flex items-center gap-1">
                <span className="font-mono text-[11px] font-bold text-[#C13A5A]">{data.reference}</span>
                <button
          ...
          {/* Support Link */}
          <a
            href="https://t.me/JB BosterCashSupport"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-1.5 text-xs text-slate-400 hover:text-[#C13A5A] py-1 font-semibold transition-colors"
          >
            <MessageCircle className="w-3.5 h-3.5 text-[#C13A5A]" />
